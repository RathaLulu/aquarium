...
class aquarium : 
    def __init__(self, json_epoch, surface):
        ...
        self.poissons = [poisson(p, RED, self.surface) for p in json_epoch['poissons']]
...
class poisson: 
    ...
    def __init__(self, json_poisson, color, surface): 
    ...
    def draw(self):
        ...
        if self.flip:
            # Corps
            pygame.draw.ellipse(self.surface, self.color, (x - body_width, y, body_width, body_height))
            # Queue
            tail = [(x, y + body_height // 2),
                    (x + tail_size, y),
                    (x + tail_size, y + body_height)]
            pygame.draw.polygon(self.surface, self.color, tail)
        else:
            # Corps
            pygame.draw.ellipse(self.surface, self.color, (x, y, body_width, body_height))
            # Queue
            tail = [(x, y + body_height // 2),
                    (x - tail_size, y),
                    (x - tail_size, y + body_height)]
